...
raw <- read.csv('log.csv', header=T, sep=',', as.is=T)
psy <- read.csv('log_id.csv', header=T, sep=',', as.is=T)
...
pres = data.frame(pi$pressure, pi$step_min, pf$pressure)
names(pres)[names(pres) == "pi.step_min"] <- "step_min"

# water potential extrapolated for every min
psy$datetime = with(psy, dmy(date) + hms(hour))
...
pres <- merge(pres, psy1, by="step_min")

#calculate air discharged (AD) in mols, uL, and the percentage of air discharged (PAD)
pres$ad_mol <- (((p_atm-(pres$pf.pressure*1000))*(Vr))/(R*temp))-(((p_atm-(pres$pi.pressure*1000))*(Vr))/(R*temp))
pres$ad_ul <- (pres$ad_mol*R*temp/(p_atm))*1000*1000*1000
...
fit.pad = try.nls(work.table=pres,model=pad~100/(1+exp(a*(psy-p50))),start=data.frame(parameter=c("a","p50"),min=c(0,-10),max=c(5,0)))
summary(fit.pad)
...
##### plot graph
library(ggplot2)
fig<-ggplot (data= pres, aes(x = psy, y = pad))+geom_point(shape = 21, size = 3) +
...
# you may comment (put # before) the next lines if you do not want text in your graph
  ...
  annotate(geom="text", x=-6, y=30, label= expression(psi[50]), color="black")+
  annotate(geom="text", x=-6, y=25, label= round(p50.pad, 2), color="royalblue")+
  annotate(geom="text", x=-6, y=20, label= round(p50_near, 2), color="red")+
  annotate(geom="text", x=-5, y=30, label= expression(psi[88]), color="black")+
  annotate(geom="text", x=-5, y=25, label= round(p88.pad, 2), color="royalblue")+
  annotate(geom="text", x=-5, y=20, label= round(p88_near, 2), color="red")+
  annotate(geom="text", x=-4, y=30, label= expression(psi[12]), color="black")+
  annotate(geom="text", x=-4, y=25, label= round(p12.pad, 2), color="royalblue")+
  annotate(geom="text", x=-4, y=20, label= round(p12_near, 2), color="red")+
...
fig_gd<-ggplot (data= pres, aes(x = psy, y = ad_ul))+geom_point(shape = 21, size = 3) +
  scale_x_continuous("Water potential (MPa)") + 
...
fig_gd_time<-ggplot (data= pres, aes(x = time, y = ad_ul))+geom_point(shape = 21, size = 3) +
